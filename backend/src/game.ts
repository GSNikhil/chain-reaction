...
function criticalMass(board: Board, x: number, y: number): number {
  ...
  let neighbors = 0;
  if (x > 0) neighbors++;
  if (x < rows - 1) neighbors++;
  if (y > 0) neighbors++;
  if (y < cols - 1) neighbors++;
  return neighbors;
}


export function placeOrb(room: Room, x: number, y: number, playerIndex: number) {
